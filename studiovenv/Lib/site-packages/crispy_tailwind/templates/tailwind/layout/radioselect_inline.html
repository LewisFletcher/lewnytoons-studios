...
{% else %}
    ...
    {% if field.label %}
        <label for="{{ field.id_for_label }}"  class="{% if label_class %}{{ label_class }}{% else %}block text-gray-700 text-sm font-bold mb-6 {% endif %}{% if not inline_class %} col-form-label{% endif %}{% if field.field.required %} requiredField{% endif %}">
            {{ field.label|safe }}
        </label>
    {% endif %}
